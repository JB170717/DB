...
--INSERT구문도 서브쿼리가 됩니다.
--실습을 위한 사본테이블 생성
CREATE TABLE EMPS AS (SELECT * FROM EMPLOYEES WHERE 1 = 2);  --EMPS테이블을 만드는데 데이터는 복제X
DESC EMPS;
SELECT * FROM EMPS;
--1ST
...
UPDATE EMPS SET FIRST_NAME='홍', SALARY=3000, COMMISSION_PCT=0.1 WHERE EMPLOYEE_ID=120;

UPDATE EMPS SET FIRST_NAME ='HONG'; -- 전부 다 바뀐다. 그러므로 WHERE절써라
ROLLBACK;
...
-----------------------------------------------------------------------------------------
-- DELETE문
-- 삭제하기 전에 SELECT로 삭제할 데이터르 꼭 확인하세요.
...
WHEN NOT MATCHED THEN -- 일치하지 않을떄 수행할 작업
    INSERT(EMPLOYEE_ID, LAST_NAME, EMAIL, HIRE_DATE, JOB_ID) 
    VALUES(E2.EMPLOYEE_ID, E2.LAST_NAME, E2.EMAIL, E2.HIRE_DATE, E2.JOB_ID)    
;
DESC EMPS;

--2ND - MERGE문으로 직접 특정 데이터에 값을 넣고자 할 때 사용할수 있음
MERGE INTO EMPS E1
USING DUAL
...
UPDATE DEPTS2 SET MANAGER_ID=100 WHERE MANAGER_ID IS NOT NULL;
SELECT *FROM DEPARTMENTS;
...
WHEN NOT MATCHED THEN
    INSERT VALUES(J2.JOB_ID,J2.JOB_TITLE,MIN_SALARY,MAX_SALARY);
