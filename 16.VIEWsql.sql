--뷰란?
--제한적인 자료를 보기위해서 미리 만들어 놓은 가상테이블 입니다.
--자주 사용되는 컬럼만 지정해 놓으면, 관리가 용이해 집니다.
--뷰는 물리적으로 데이터가 저장된 형태는 아니고, 원본테이블을 기반으로한 논리적 테이블 입니다.(가짜테이블이라는 것)
...
GROUP BY JOB_TITLE; --직무네임별 직원수

--뷰를 수정은 OR REPLACE를 붙이면 됩니다.
CREATE OR REPLACE VIEW VIEW_EMP_JOB
AS (
    SELECT E.EMPLOYEE_ID,
           FIRST_NAME||LAST_NAME AS NAME,
...
SELECT * FROM VIEW_EMP;
INSERT INTO VIEW_EMP(EMP_ID,NAME) VALUES(250,'HONG'); --EMP-NAME, NAME가상열이기 떄문에 INSERT 불가능 
INSERT INTO VIEW_EMP(JOB_ID, SALARY) VALUES('IT_FROG',5000); --물리적테이블의 NOT NULL제약을 위배하기 떄문에, 불가능
...
UPDATE VIEW_EMP SET DEPARTMENT_ID=100 WHERE EMPLOYEE_ID=105; --DEPARTMENT_ID는 60, 70 ,80이어야함
